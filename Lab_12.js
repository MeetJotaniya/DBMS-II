...
db.Student.aggregate([{$match:{CITY:"Baroda"}},{$group:{_id:null,totalStudents:{$sum:1}}}])
//10 Display average fees of Rajkot city. 
db.Student.aggregate([{$match:{CITY:"Rajkot"}},{$group:{_id:null,averageFees:{$avg:"$FEES"}}}])
//11 Count the number of male and female students in each Department 
db.Student.aggregate([{$group:{_id:{Department:"$DEPARTMENT",Gender:"$GENDER"},totalStudent:{$sum:1}}}])
...
db.Student.aggregate([{$match:{CITY:"Rajkot"}},{$group:{_id:"$DEPARTMENT",averageFees:{$avg:"$FEES"}}}])
//19 Group by Sem and calculate both the total and average Fees, then sort by total fees in descending order.
db.Student.aggregate([{$group:{_id:"$SEM",totalFees:{$sum:"$FEES"},averageFees:{$avg:"$FEES"}}},{$sort:{totalFees:-1}}])
...
db.Stock.aggregate([{$group:{_id:"$sector",averageProfit:{$avg:"$profit"}}}])
//5 Get the count of companies in each sector.
db.Stock.aggregate([{$group:{_id:"$sector",countCompanies:{$sum:1}}}])
...
db.Stock.aggregate([
    ...
    },
    {
      $lookup: {
        ...
        as: "companiesWithHighEPS"
      }
    },
    {
      $unwind: "$companiesWithHighEPS"
    },
    {
      $replaceRoot: { newRoot: "$companiesWithHighEPS" }
    }
  ]);
...
db.Stock.aggregate([{$group:{_id:"$sector",totalProfit:{$sum:"$profit"},totalSales:{$sum:"$sales"}}}])
//13 Sort companies by profit in descending order.(Use: Aggregate) 
db.Stock.aggregate([{$group:{_id:"$company"}},{$sort:{profit:-1}}])
//14 Find the average ROE across all companies. 
db.Stock.aggregate([{$group:{_id:"$company",averageROE:{$avg:"$roe"}}}])
...
db.Stock.aggregate([{$match:{profit:{$gt:30000}}},{$group:{_id:null,countCompany:{$sum:1}}}]) 
...
db.Stock.aggregate([{$group:{_id:"$company",totalSales:{$sum:"$sales"},totalProfit:{$sum:"$profit"}}}])
...
db.Stock.aggregate([{$group:{_id:"$sector",totalCompany:{$sum:1}}}])
...
db.Stock.aggregate([{$group:{_id:"$sector",totalSales:{$sum:"$sales"}}}])
